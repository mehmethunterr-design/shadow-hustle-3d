...
func _physics_process(delta: float) -> void:
	...
	if player_ref == null and world_ref:
		player_ref = world_ref.player
...
func _can_see_player() -> bool:
	...
	var to := player_ref.global_position + Vector3.UP * 1.1
...
func _attack(delta: float) -> void:
	...
	if attack_timer <= 0.0:
		...
		if world_ref:
			world_ref.show_info("%s vurdu! -%d HP" % [npc_name, attack_damage])
...
func _retreat(delta: float) -> void:
	...
	if away.length() < 0.01:
		away = Vector3.RIGHT
	var retreat_target := global_position + away.normalized() * 8.0
...
func _update_civilian(delta: float) -> void:
	var danger := _nearest_hostile(7.0)
	if danger:
...
func _nearest_hostile(max_distance: float):
	var nearest = null
	var nearest_distance := max_distance
	for enemy in get_tree().get_nodes_in_group("hostile_npc"):
		if enemy == self:
			continue
...
func _face_target(target: Vector3, delta: float) -> void:
	...
	var desired := global_transform.looking_at(flat_target, Vector3.UP).basis.get_euler().y
